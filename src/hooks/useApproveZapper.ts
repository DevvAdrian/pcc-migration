...
import { BNB_TICKER, NEW_TICKER, OLD_TICKER, ZAPPER_ROUTER_ADDR } from '../utils/constants';
...
function useApproveZapper(zappingToken: string): [ApprovalState, () => Promise<void>] {
  ...
  let token: ERC20;
  if (zappingToken === BNB_TICKER) token = tombFinance.BNB;
  else if (zappingToken === NEW_TICKER) token = tombFinance.NEW;
  else if (zappingToken === OLD_TICKER) token = tombFinance.OLD;
  ...
  const approvalState: ApprovalState = useMemo(() => {
    ...
    if (token === tombFinance.BNB) return ApprovalState.APPROVED;
